...
"""
function pitchshift(tsm::AT, x::AbstractVector{T}, semitones::Real; fixlen::Bool=true) where {T<:Number,AT<:AbstractTimeScaleModifier}
    rate = 2 ^ (semitones / 12)
...
"""
function timestretch(tsm::AT, x::AbstractVector{T}, speed::Real; fixlen::Bool=true) where {T<:Number,AT<:AbstractTimeScaleModifier}
